...
idx_file="./indices/GCF_018294505.1/GCF_018294505.1.rna.idx"
input_dir="./reads/raw"
output_dir="./quant/chinese_spring"
mkdir -p "$output_dir"

# Loop through all R1 read files in the input directory
for fastq in "$input_dir"/*-R1.trimmed.fastq.gz; do

    # Define the sample ID
    sample_ID=$(basename "$fastq" -R1.trimmed.fastq.gz)

    # Define the read files
    R1="$input_dir/${sample_ID}-R1.trimmed.fastq.gz"
    R2="$input_dir/${sample_ID}-R2.trimmed.fastq.gz"
