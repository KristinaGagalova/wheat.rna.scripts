...
# Avoid extreme y-values breaking the plot
y_min <- max(min(go_results$p.adjust, na.rm = TRUE), 1e-10)
y_max <- min(max(go_results$p.adjust, na.rm = TRUE), 1)
y_breaks <- scales::log_breaks()(c(y_min, y_max))  # Log-transformed breaks

# Generate Bubble Plot with Correct Y-Axis Scaling
p <- ggplot(go_results, aes(
  ...
  # Dynamic y-axis scaling with **correct inversion**
